...
RESPONSE_CODE=$(curl -s -o /dev/null -w "%{http_code}" https://juhwan.gq/profile)
...
then
    ...
    CURRENT_PORT=$(curl -s https://juhwan.gq/profile)
fi
...
sudo docker pull juhwan0815/board:latest
